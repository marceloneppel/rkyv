...
use crate::{
    primitive::ArchivedUsize, ser::{Allocator, Writer, WriterExt as _}, tuple::*, Archive, ArchivePointee, ArchiveUnsized, ArchivedMetadata, CopyOptimization, Deserialize, DeserializeUnsized, LayoutRaw, Portable, Serialize, SerializeUnsized
};
    ...
    D: Fallible + ?Sized,
{
    unsafe fn deserialize_unsized(
        ...
        for (i, item) in self.iter().enumerate() {
            out.cast::<U>().add(i).write(item.deserialize(deserializer)?);
        }
        Ok(())
    }
